...
class TemplateProcessor
{
    ...
    public function setImageValue($search, $replace, $limit = self::MAXIMUM_REPLACEMENTS_DEFAULT)
    {
        ...
        if (isset($replacesList[0]['align']) and in_array($replacesList[0]['align'], array('left','right','center'))) {
            $imgTpl = str_replace('{{align}}', $replacesList[0]['align'], '<w:p w:rsidR="0021268F" w:rsidRDefault="00495C2C" w:rsidP="00972DB5"><w:pPr><w:jc w:val="{{align}}"/></w:pPr><w:r><w:bookmarkStart w:id="0" w:name="_GoBack"/><w:pict><v:shape type="#_x0000_t75" style="width:{WIDTH};height:{HEIGHT}"><v:imagedata r:id="{RID}" o:title=""/></v:shape></w:pict></w:r><w:bookmarkEnd w:id="0"/></w:p>');
        } else {
