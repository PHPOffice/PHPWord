...
{
    /**
     * PHPWord_ReferenceHelper instance
     *
     * @var PHPWord_ReferenceHelper
     */
    private $_referenceHelper = NULL;

    /**
     * PHPWord_Reader_Word2007_Theme instance
     *
     * @var PHPWord_Reader_Word2007_Theme
     */
    ...
    public function __construct() {
        $this->_readFilter = new PHPWord_Reader_DefaultReadFilter();
        $this->_referenceHelper = PHPWord_ReferenceHelper::getInstance();
    }
...
//          throw new PHPWord_Reader_Exception($zipClass . " library is not enabled");
//      }
    ...
    /**
     * Reads names of the worksheets from a file, without parsing the whole file to a PHPWord object
     *
     * @param   string      $pFilename
     * @throws  PHPWord_Reader_Exception
     */
            ...
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $res = $xml->open('zip://'.PHPWord_Shared_File::realpath($pFilename).'#'."$dir/$fileWorksheet");
                        ...
                        $tmpInfo['lastColumnLetter'] = PHPWord_Cell::stringFromColumnIndex($tmpInfo['lastColumnIndex']);
...
//              echo 'Formula is '.$sharedFormulas[$instance]['formula'].PHP_EOL;
                $master = PHPWord_Cell::coordinateFromString($sharedFormulas[$instance]['master']);
                $current = PHPWord_Cell::coordinateFromString($r);

                $difference = array(0, 0);
                $difference[0] = PHPWord_Cell::columnIndexFromString($current[0]) - PHPWord_Cell::columnIndexFromString($master[0]);
...
//              echo 'Adjusted Formula is '.$value.PHP_EOL;
            ...
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    if (is_object($xmlTheme)) {
                        ...
                        self::$_theme = new PHPWord_Reader_Word2007_Theme($themeName,$colourSchemeName,$themeColours);
                    }
                    break;
            }
        }
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    if (isset($xmlStrings) && isset($xmlStrings->si)) {
                        foreach ($xmlStrings->si as $val) {
                            if (isset($val->t)) {
                                $sharedStrings[] = PHPWord_Shared_String::ControlCharacterOOXML2PHP( (string) $val->t );
                            } elseif (isset($val->r)) {
                    ...
                    if (!$this->_readDataOnly && $xmlStyles) {
                        foreach ($xmlStyles->cellXfs->xf as $xf) {
                            $numFmt = PHPWord_Style_NumberFormat::FORMAT_GENERAL;
                            ...
                            if ($xf["numFmtId"]) {
                                ...
                                if ((int)$xf["numFmtId"] < 164) {
                                    $numFmt = PHPWord_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]);
                                }
                            }
                            $quotePrefix = false;
                    ...
                    if (!$this->_readDataOnly && $xmlStyles) {
                        ...
                        if ($xmlStyles->dxfs) {
                            foreach ($xmlStyles->dxfs->dxf as $dxf) {
                                $style = new PHPWord_Style(FALSE, TRUE);
                    ...
                    if ($xmlWorkbook->workbookPr) {
                        PHPWord_Shared_Date::setExcelCalendar(PHPWord_Shared_Date::CALENDAR_WINDOWS_1900);
                        if (isset($xmlWorkbook->workbookPr['date1904'])) {
                            if (self::boolean((string) $xmlWorkbook->workbookPr['date1904'])) {
                                PHPWord_Shared_Date::setExcelCalendar(PHPWord_Shared_Date::CALENDAR_MAC_1904);
                            }
                        }
                    }
                                ...
                                foreach ($xmlSheet->cols->col as $col) {
                                    for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
                                        if ($col["style"] && !$this->_readDataOnly) {
                                            $docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setXfIndex(intval($col["style"]));
                                        }
                                        if (self::boolean($col["bestFit"])) {
                                            //$docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setAutoSize(TRUE);
                                        }
                                        if (self::boolean($col["hidden"])) {
                                            $docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setVisible(FALSE);
                                        }
                                        if (self::boolean($col["collapsed"])) {
                                            $docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setCollapsed(TRUE);
                                        }
                                        if ($col["outlineLevel"] > 0) {
                                            $docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setOutlineLevel(intval($col["outlineLevel"]));
                                        }
                                        $docSheet->getColumnDimension(PHPWord_Cell::stringFromColumnIndex($i))->setWidth(floatval($col["width"]));
                                ...
                                foreach ($xmlSheet->sheetData->row as $row) {
                                    ...
                                    foreach ($row->c as $c) {
                                        ...
                                        if ($this->getReadFilter() !== NULL) {
                                            $coordinates = PHPWord_Cell::coordinateFromString($r);
    ...
    //                                  echo 'Value is '.$value.'<br />';
                                        ...
                                        // Rich text?
                                        if ($value instanceof PHPWord_RichText && $this->_readDataOnly) {
                            ...
                            if (!$this->_readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                foreach ($xmlSheet->conditionalFormatting as $conditional) {
                                    foreach ($conditional->cfRule as $cfRule) {
                                        if (
                                            (
                                                (string)$cfRule["type"] == PHPWord_Style_Conditional::CONDITION_NONE ||
                                                (string)$cfRule["type"] == PHPWord_Style_Conditional::CONDITION_CELLIS ||
                                                (string)$cfRule["type"] == PHPWord_Style_Conditional::CONDITION_CONTAINSTEXT ||
                                                (string)$cfRule["type"] == PHPWord_Style_Conditional::CONDITION_EXPRESSION
                                ...
                                foreach ($conditionals as $ref => $cfRules) {
                                    ...
                                    foreach ($cfRules as $cfRule) {
                                        $objConditional = new PHPWord_Style_Conditional();
                                    ...
                                    $aReferences = PHPWord_Cell::extractAllCellReferencesInRange($ref);
                            ...
                            if ($xmlSheet && $xmlSheet->autoFilter && !$this->_readDataOnly) {
                                ...
                                foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                    ...
                                    if ($filterColumn->filters) {
                                        $column->setFilterType(PHPWord_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER);
                                        ...
                                        if ((isset($filters["blank"])) && ($filters["blank"] == 1)) {
                                            ...
                                            ->setRuleType(PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_FILTER);
                                        }
                                        ...
                                        foreach ($filters->dateGroupItem as $dateGroupItem) {
                                            ...
                                            ->setRuleType(PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DATEGROUP);
                                        }
                                    }
                                    //  Check for custom filters
                                    if ($filterColumn->customFilters) {
                                        $column->setFilterType(PHPWord_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER);
                                        $customFilters = $filterColumn->customFilters;
                                        //  Custom filters can an AND or an OR join;
                                        //      and there should only ever be one or two entries
                                        if ((isset($customFilters["and"])) && ($customFilters["and"] == 1)) {
                                            $column->setJoin(PHPWord_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND);
                                        }
                                        foreach ($customFilters->customFilter as $filterRule) {
                                            ...
                                            ->setRuleType(PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);
                                        }
                                    }
                                    //  Check for dynamic filters
                                    if ($filterColumn->dynamicFilter) {
                                        $column->setFilterType(PHPWord_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER);
                                        ...
                                        foreach ($filterColumn->dynamicFilter as $filterRule) {
                                            ...
                                            ->setRuleType(PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
                                    ...
                                    if ($filterColumn->top10) {
                                        $column->setFilterType(PHPWord_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER);
                                        //  We should only ever have one top10 filter
                                        foreach ($filterColumn->top10 as $filterRule) {
                                            $column->createRule()->setRule(
                                                (((isset($filterRule["percent"])) && ($filterRule["percent"] == 1))
                                                    ? PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT
                                                    : PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE
                                                ),
                                                (string) $filterRule["val"],
                                                (((isset($filterRule["top"])) && ($filterRule["top"] == 1))
                                                    ? PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP
                                                    : PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM
                                                )
                                            )
                                            ->setRuleType(PHPWord_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_TOPTENFILTER);
                                        }
                                    }
                                }
                            }
                            ...
                            if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->_readDataOnly) {
                                foreach ($xmlSheet->rowBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak("A$brk[id]", PHPWord_Worksheet::BREAK_ROW);
                                    }
                                }
                            }
                            if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->_readDataOnly) {
                                foreach ($xmlSheet->colBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak(PHPWord_Cell::stringFromColumnIndex((string) $brk["id"]) . "1", PHPWord_Worksheet::BREAK_COLUMN);
                                    }
                                }
                            }
                            ...
                            if ($xmlSheet && $xmlSheet->dataValidations && !$this->_readDataOnly) {
                                foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
                                    ...
                                    foreach($rangeSet as $range) {
                                        ...
                                        $aReferences = PHPWord_Cell::extractAllCellReferencesInRange($stRange);
                                        foreach ($aReferences as $reference) {
                                            // Create validation
                            ...
                            if (!$this->_readDataOnly) {
                                ...
                                if ($xmlSheet && $xmlSheet->hyperlinks) {
                                    foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
                                        ...
                                        foreach (PHPWord_Cell::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
                            ...
                            if (!$this->_readDataOnly) {
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    // Load comments file
                                    $relPath = PHPWord_Shared_File::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            foreach ($shapes as $idx => $shape) {
                                                ...
                                                $hfImages[ (string)$shape['id'] ] = new PHPWord_Worksheet_HeaderFooterDrawing();
                                                if (isset($imageData['title'])) {
                                                    $hfImages[ (string)$shape['id'] ]->setName( (string)$imageData['title'] );
                                                }

                                                $hfImages[ (string)$shape['id'] ]->setPath("zip://".PHPWord_Shared_File::realpath($pFilename)."#" . $drawings[(string)$imageData['relid']], false);
                                        ...
                                        if ($xmlDrawing->oneCellAnchor) {
                                            foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing = new PHPWord_Worksheet_Drawing;
                                                    ...
                                                    $objDrawing->setPath("zip://".PHPWord_Shared_File::realpath($pFilename)."#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
                                                    $objDrawing->setCoordinates(PHPWord_Cell::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1));
                                                    $objDrawing->setOffsetX(PHPWord_Shared_Drawing::EMUToPixels($oneCellAnchor->from->colOff));
                                                    $objDrawing->setOffsetY(PHPWord_Shared_Drawing::EMUToPixels($oneCellAnchor->from->rowOff));
                                                    $objDrawing->setResizeProportional(false);
                                                    $objDrawing->setWidth(PHPWord_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx")));
                                                    $objDrawing->setHeight(PHPWord_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy")));
                                                    if ($xfrm) {
                                                        $objDrawing->setRotation(PHPWord_Shared_Drawing::angleToDegrees(self::array_item($xfrm->attributes(), "rot")));
                                                    }
                                                    if ($outerShdw) {
                                                        $shadow = $objDrawing->getShadow();
                                                        $shadow->setVisible(true);
                                                        $shadow->setBlurRadius(PHPWord_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "blurRad")));
                                                        $shadow->setDistance(PHPWord_Shared_Drawing::EMUTopixels(self::array_item($outerShdw->attributes(), "dist")));
                                                        $shadow->setDirection(PHPWord_Shared_Drawing::angleToDegrees(self::array_item($outerShdw->attributes(), "dir")));
                                                    ...
                                                    $coordinates    = PHPWord_Cell::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1);
                                                    $offsetX        = PHPWord_Shared_Drawing::EMUToPixels($oneCellAnchor->from->colOff);
                                                    $offsetY        = PHPWord_Shared_Drawing::EMUToPixels($oneCellAnchor->from->rowOff);
                                                    $width          = PHPWord_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx"));
                                                    $height         = PHPWord_Shared_Drawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy"));
                                                }
                                            }
                                        }
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing->setPath("zip://".PHPWord_Shared_File::realpath($pFilename)."#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
                                                    $objDrawing->setCoordinates(PHPWord_Cell::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1));
                                                    $objDrawing->setOffsetX(PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->from->colOff));
                                                    $objDrawing->setOffsetY(PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->from->rowOff));
                                                    $objDrawing->setResizeProportional(false);

                                                    $objDrawing->setWidth(PHPWord_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cx")));
                                                    $objDrawing->setHeight(PHPWord_Shared_Drawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cy")));
                                                    ...
                                                    $fromCoordinate = PHPWord_Cell::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1);
                                                    $fromOffsetX    = PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->from->colOff);
                                                    $fromOffsetY    = PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->from->rowOff);
                                                    $toCoordinate   = PHPWord_Cell::stringFromColumnIndex((string) $twoCellAnchor->to->col) . ($twoCellAnchor->to->row + 1);
                                                    $toOffsetX      = PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->to->colOff);
                                                    $toOffsetY      = PHPWord_Shared_Drawing::EMUToPixels($twoCellAnchor->to->rowOff);
                            ...
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                ...
                                if ((string)$definedName['localSheetId'] != '') {
                                            ...
                                            if ($mapSheetId[(integer) $definedName['localSheetId']] !== null) {
                                                ...
                                                if (count($range) == 2) {
                                                    ...
                                                    if ($worksheet = $docSheet->getParent()->getSheetByName($range[0])) {
                                                        ...
                                                        $excel->addNamedRange( new PHPWord_NamedRange((string)$definedName['name'], $worksheet, $extractedRange, true, $scope) );
                                                    }
                                                }
                                            }
                                            break;
                                    ...
                                    if (strpos( (string)$definedName, '!' ) !== false) {
                                        // Extract sheet name
                                        $extractedSheetName = PHPWord_Worksheet::extractSheetTitle( (string)$definedName, true );
                                    ...
                                    if ($locatedSheet !== NULL) {
                                        $excel->addNamedRange( new PHPWord_NamedRange((string)$definedName['name'], $locatedSheet, $extractedRange, false) );
                                    }
                                }
                            }
                        }
                ...
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->_includeCharts) {
                            ...
                            $objChart = PHPWord_Reader_Word2007_Chart::readChart($chartElements,basename($chartEntryRef,'.xml'));
...
//                                  var_dump($chartDetails[$chartPositionRef]);
    ...
    private static function _readColor($color, $background=FALSE) {
        if (isset($color["rgb"])) {
            ...
            return PHPWord_Style_Color::indexedColor($color["indexed"]-7,$background)->getARGB();
        } else if (isset($color["theme"])) {
            if (self::$_theme !== NULL) {
                $returnColour = self::$_theme->getColourByIndex((int)$color["theme"]);
                if (isset($color["tint"])) {
                    $tintAdjust = (float) $color["tint"];
                    $returnColour = PHPWord_Style_Color::changeBrightness($returnColour, $tintAdjust);
                }
...
//      }
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->u) && !isset($style->font->u["val"])) {
                $docStyle->getFont()->setUnderline(PHPWord_Style_Font::UNDERLINE_SINGLE);
        ...
        if (isset($style->border)) {
            ...
            if (!$diagonalUp && !$diagonalDown) {
                $docStyle->getBorders()->setDiagonalDirection(PHPWord_Style_Borders::DIAGONAL_NONE);
            } elseif ($diagonalUp && !$diagonalDown) {
                $docStyle->getBorders()->setDiagonalDirection(PHPWord_Style_Borders::DIAGONAL_UP);
            } elseif (!$diagonalUp && $diagonalDown) {
                $docStyle->getBorders()->setDiagonalDirection(PHPWord_Style_Borders::DIAGONAL_DOWN);
            } else {
                $docStyle->getBorders()->setDiagonalDirection(PHPWord_Style_Borders::DIAGONAL_BOTH);
            }
        ...
        if (isset($style->protection)) {
            if (isset($style->protection['locked'])) {
                if (self::boolean((string) $style->protection['locked'])) {
                    $docStyle->getProtection()->setLocked(PHPWord_Style_Protection::PROTECTION_PROTECTED);
                } else {
                    $docStyle->getProtection()->setLocked(PHPWord_Style_Protection::PROTECTION_UNPROTECTED);
                }
            }

            if (isset($style->protection['hidden'])) {
                if (self::boolean((string) $style->protection['hidden'])) {
                    $docStyle->getProtection()->setHidden(PHPWord_Style_Protection::PROTECTION_PROTECTED);
                } else {
                    $docStyle->getProtection()->setHidden(PHPWord_Style_Protection::PROTECTION_UNPROTECTED);
                }
            }
        }
    ...
    private function _parseRichText($is = null) {
        $value = new PHPWord_RichText();

        if (isset($is->t)) {
            $value->createText( PHPWord_Shared_String::ControlCharacterOOXML2PHP( (string) $is->t ) );
        } else {
            foreach ($is->r as $run) {
                if (!isset($run->rPr)) {
                    $objText = $value->createText( PHPWord_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );

                } else {
                    $objText = $value->createTextRun( PHPWord_Shared_String::ControlCharacterOOXML2PHP( (string) $run->t ) );
                    ...
                    if (isset($run->rPr->color)) {
                        $objText->getFont()->setColor( new PHPWord_Style_Color( self::_readColor($run->rPr->color) ) );
                    }
                    ...
                    if (isset($run->rPr->u) && !isset($run->rPr->u["val"])) {
                        $objText->getFont()->setUnderline(PHPWord_Style_Font::UNDERLINE_SINGLE);
    ...
    private static function toCSSArray($style) {
        ...
        foreach ($temp as $item) {
            ...
            if (strpos($item[1], 'pt') !== false) {
                ...
                $item[1] = PHPWord_Shared_Font::fontSizeToPixels($item[1]);
            }
            if (strpos($item[1], 'in') !== false) {
                $item[1] = str_replace('in', '', $item[1]);
                $item[1] = PHPWord_Shared_Font::inchSizeToPixels($item[1]);
            }
            if (strpos($item[1], 'cm') !== false) {
                $item[1] = str_replace('cm', '', $item[1]);
                $item[1] = PHPWord_Shared_Font::centimeterSizeToPixels($item[1]);
            }
